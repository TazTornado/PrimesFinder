...
////////////////////* Handle program parameters *////////////////////
	...
	if((argc < 4) || (argc > 5)){       // arguments are too few or too many
		printf("Usage: ./Inodes lb ub numOfChildren pipefd\n(pipefd is optional)\n");
		exit(1);
	}
	// printf("\nargc = %d\n", argc);
	...
	// int writefd = -1;
	printf("This Inode's args: lb = %d, ub = %d, bfactor = %d\n", lb, ub, bfactor);
	...
	if(ub < lb){
		...
		printf("Usage: ./Inodes -l lb -u ub -w numOfChildren\n");
		exit(EXIT_FAILURE);
	}
